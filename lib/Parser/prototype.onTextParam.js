...
module.exports = function onTextParam(part) {
  ...
  part.on('readable', function onBytesAvailable() {
    ...
    // If field contains dot, assume it is dot notation field
    if (field.indexOf('.') > -1) {
      ...
      if (self.req.body[fieldNames[0]])
        iterator = self.req.body[fieldNames[0]];


      while (counter < fieldNames.length) {
        if (i != fieldNames.length - 1) {
          iterator[fieldNames[counter]] = {};
        } else {
          if (!self.multifields[field]) {
            iterator[fieldNames[counter]] = value;
            //Using default skipper benavior

          } else {
            // Run into second time
            if (self.multifields[field])
              iterator[fieldNames[counter]].push(value);
            else {
              // Init array
              iterator[fieldNames[counter]] = [iterator[fieldNames[counter]]];
    ...
    } else if (self.req.body[field]) {

      // Default skipper behavior
